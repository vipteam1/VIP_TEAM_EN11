...
function run(msg, matches)
    ...
    local data = load_data(_config.moderation.data)
    local receiver = get_receiver(msg)
			if matches[1] == 'sabout' and matches[2] == 'group' and is_realm(msg) then
				local target = matches[3]
				local about = matches[4]
				return set_description(msg, data, target, about)
			end
			if matches[1] == 'sabout' and matches[2] == 'sgroup'and is_realm(msg) then
		...
		if matches[1] == 'sname' and is_realm(msg) then
		...
		if matches[1] == 'support' and matches[2] then
			if string.match(matches[2], '^%d+$') then
				...
				print("User "..support_id.."✅ تم ترقية المجموعه الى سوبر ✅")
				support_add(support_id)
				return "User ["..support_id.."] ✅ المجموعه بلفعل سوبر في المجموعه ✅"
			else
				local member = string.gsub(matches[2], "@", "")
				local receiver = get_receiver(msg)
