...
local function kick_ban_res(extra, success, result)
      ...
      local get_cmd = extra.get_cmd
       if get_cmd == "kick" then
         if member_id == from_id then
            send_large_msg(receiver, "❌ لايمكن لك ان تحظر او تطرد نفسك ❌")
			return
         end
         if is_momod2(member_id, chat_id) and not is_admin2(sender) then
            send_large_msg(receiver, "❌ لايمكن لك ان تحظر او تطرد المشرفون ❌")
			return
         end
      ...
      elseif get_cmd == 'ban' then
        ...
        send_large_msg(receiver, 'تم حظرك 🚷 @'..member..' ['..member_id..'] من االمجموعه')
        ban_user(member_id, chat_id)
      elseif get_cmd == 'unban' then
        send_large_msg(receiver, 'تم الغاء الحظر ❗️ @'..member..' ['..member_id..'] من االمجموعه')
        local hash =  'banned:'..chat_id
        redis:srem(hash, member_id)
        return 'User '..user_id..' unbanned'
      elseif get_cmd == 'banall' then
        send_large_msg(receiver, 'تم حظرك/ج عام  📛 @'..member..' ['..member_id..'] في جميع  المجموعه')
        banall_user(member_id)
      elseif get_cmd == 'unbanall' then
        send_large_msg(receiver, 'تم الغاء حظرك/ج عام  🚸 @'..member..' ['..member_id..'] في جميع  المجموعه')
  ...
  if matches[1]:lower() == 'ban' then-- /ban
    ...
    elseif string.match(matches[2], '^%d+$') then
        ...
        if not is_admin1(msg) and is_momod2(matches[2], msg.to.id) then
          	return "❌ لايمكن لك ان تحظر او تطرد المشرفون ❌"
        end
        if tonumber(matches[2]) == tonumber(msg.from.id) then
          	return "❌ لايمكن لك ان تحظر او تطرد نفسك ❌"
        end
        local print_name = user_print_name(msg.from):gsub("‮", "")
	    local name = print_name:gsub("_", "")
		local receiver = get_receiver(msg)
        savelog(msg.to.id, name.." ["..msg.from.id.."] تم حظرك من المجموعه ".. matches[2])
        ban_user(matches[2], msg.to.id)
		send_large_msg(receiver, 'تم ['..matches[2]..'] حظرك من المجموعه')
      else
		local cbres_extra = {
		chat_id = msg.to.id,
		get_cmd = 'ban',
...
if matches[1]:lower() == 'kick' then
    ...
    local chat_id = msg.to.id
		local print_name = user_print_name(msg.from):gsub("‮", "")
		local name = print_name:gsub("_", "")
		savelog(msg.to.id, name.." ["..msg.from.id.."] تم طردك ".. matches[2])
		kick_user(user_id, chat_id)
	else
		local cbres_extra = {
			chat_id = msg.to.id,
    ...
    elseif string.match(matches[2], '^%d+$') then
	    ...
	    local name = print_name:gsub("_", "")
		local receiver = get_receiver(msg)
        savelog(msg.to.id, name.." ["..msg.from.id.."] تم حظرك عام ".. matches[2])
        banall_user(matches[2])
		send_large_msg(receiver, 'تم حظرك ['..matches[2]..'] عام من جميع المجموعات')
      else
		local cbres_extra = {
		chat_id = msg.to.id,
		get_cmd = 'banall',
 ...
 if matches[1]:lower() == 'unbanall' then-- /ban
    ...
    elseif string.match(matches[2], '^%d+$') then
	    ...
	    local name = print_name:gsub("_", "")
		local receiver = get_receiver(msg)
        savelog(msg.to.id, name.." ["..msg.from.id.."] تم الغاء الحظر العام 🚷  من البوت ".. matches[2])
        unbanall_user(matches[2])
		send_large_msg(receiver, ' ['..matches[2]..'] تم الغاء الحظر العام 🚷  من البوت')
      else
		local cbres_extra = {
		chat_id = msg.to.id,
		get_cmd = 'unbanall',
...
if matches[1]:lower() == 'unban' then-- /ban
    ...
    elseif string.match(matches[2], '^%d+$') then
	    ...
	    local name = print_name:gsub("_", "")
		local receiver = get_receiver(msg)
        savelog(msg.to.id, name.." ["..msg.from.id.."] تم ال(❌)غاء الحظر عنك  ".. matches[2])
        unbanall_user(matches[2])
		send_large_msg(receiver, ' ['..matches[2]..'] تم ال(❌)غاء الحظر عنك ')
      else
		local cbres_extra = {
		chat_id = msg.to.id,
		get_cmd = 'unban',
